...
- **Expert rare disease specialists**: 54.6% Recall@1 / 65.6% Recall@5 in the DeepRare 2026 head-to-head study (5 physicians with ≥10 years rare-disease practice, on 163 Xinhua Hospital cases, HPO input)
...
- **DeepRare** (Zhao et al., *Nature* Feb 2026; arXiv:2506.20430): 57.18% HPO-only Recall@1 averaged across public benchmarks; 64.4% on the 163-case Xinhua hospital cohort (vs. physicians at 54.6%); 69.1% multi-modal with HPO + genetic data. Six-agent institutional system with HPO + OMIM + Orphanet + web tools, default DeepSeek-V3 host. Evaluates on RareBench + MIMIC-IV-Rare + MyGene2 + in-house Xinhua/Hunan hospital cohorts — not on RareArena. Different cohorts; included as a ballpark reference, not a head-to-head comparison.
...
**DeepRare (57.18% HPO-only avg / 64.4% on the Xinhua hospital cohort) — a reference point for institutional-grade work.** What a serious research lab can build when the medical system is fully on their side: a 6-agent system with HPO + OMIM + Orphanet + 40+ specialized tools, evaluated on cases including newly-collected hospital cohorts that were never published online. Their evaluation cohorts differ from ours, so the numbers aren't directly comparable — but they're the closest published reference point for what's achievable on this class of task with full institutional access.
...
The space between the physician floor (~26%) and the institutional ceiling (DeepRare's mid-60s on hospital cases) is what this benchmark explores. Each condition tests whether a specific tool or technique — one that a consumer can actually access — moves the needle within that gap.
...
6. **How far can freely available MCPs get us?** — HPO alone vs. HPO + PubMed, with DeepRare's institutional equivalent as a reference
...
| Condition         | Model                                   | Sample | Backend   |
| ----------------- | --------------------------------------- | ------ | --------- |
| `opus-web-search` | Opus 4.6 + web*search + web*fetch tools | 200    | Streaming |
