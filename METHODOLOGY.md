...
| **DeepRare (Nature 2026)** | 54.67% | Institutional 6-agent system with HPO+OMIM+Orphanet, deployed at 600+ hospitals |
...
Approaching the **DeepRare ceiling** (54.67% Top-1 on RareArena RDS) is a more ambitious question: what can a consumer-accessible setup do relative to the best institutional AI, and where's the gap?
...
| Condition | N | What it represents |
|---|---|---|
| `opus-agent-hpo-pubmed` | 500 | Claude Opus running inside the Agent SDK with live access to an HPO/Orphanet MCP server and the public PubMed MCP (35M articles, free). Models decide *when* to consult each tool. This is the "patient uses Claude Code as a diagnostic research assistant" scenario — available today to anyone willing to install a CLI and a couple of MCP servers. |

**Why 500:** each case runs an agent loop with multiple tool calls, averaging 30-90 seconds of Opus time at ~$0.15-$0.30 per case. An initial 100-case pass showed a positive signal (+12.6pp Top-5 over injection-style grounding), motivating an expansion to 500 for publication-grade precision on the headline tier-5 result.
...
| `opus-debate-team-v2` | 300 | **Delphi-style redesign**, research-grounded. Three reasoning-style specialists (pattern matcher, mechanism reasoner, differential excluder), each with full HPO+PubMed+WebSearch tool access. Two independent rounds with aggregated anonymized feedback between them. Synthesis weighted by convergence *and* preserves stood-firm dissent. Models what an actual rare-disease case conference does. |

**Why 300:** each case spawns 3 (v1) or 6 (v2) subagent invocations with tool use, making wallclock and cost the binding constraint (~$0.20-$0.90 per case). 300 cases provides ~3% standard error on a 50% recall — enough to detect a 6-8pp effect at 80% power, which is the resolution needed for "v2 vs. v1" and "v2 vs. DeepRare" comparisons.
...
| 5 | opus-agent-hpo-pubmed | 500 | Publication-grade tier-5 number | Does agentic tool use beat static injection? |
| 6 | opus-debate-team (v1) | 300 | Detect 6-8pp at agent cost ceiling | Does naive multi-specialist committee help? |
| 6 | opus-debate-team-v2 | 300 | Detect 6-8pp at agent cost ceiling | Does a research-grounded Delphi architecture help? |
