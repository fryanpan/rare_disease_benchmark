...
TASKS = {
    ...
    "RDS_postcutoff": {
        "description": "Audit Phase 5: post-training-cutoff hold-out (PubMed case reports published Nov 2025+, after Opus 4.6's Aug 2025 training data cutoff)",
        "input_fields": ["case_report"],
    },
    "RDS_postcutoff_clean": {
        "description": "Post-cutoff hold-out, N=373, leakage-rescued via Haiku rewrite (clean per pattern + judge filter)",
        "input_fields": ["case_report"],
    },
}
...
CONDITIONS = {
    ...
    "opus-thinking": Condition(
        ...
        # Adaptive thinking can consume most of the budget before any text is
        # emitted; 4096 truncated mid-thinking on ~6% of opus and ~37% of sonnet
        # cases on the initial run. 24576 leaves headroom for thinking + answer.
        max_tokens=24576,
        sample_n=500,
    ),
