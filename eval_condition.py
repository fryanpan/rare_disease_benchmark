...
# ── Evaluation prompts ───────────────────────────────────────────────────────
#
# The two prompts below are adapted from `eval/eval_updated.py` in the RareArena
# repository (https://github.com/zhao-zy15/RareArena) by Zhao et al.
# (Lancet Digital Health 2026). Used here under CC BY-NC-SA 4.0, which is the
# license that applies to this benchmark harness as a result of this borrowing.
# See the repo root LICENSE for full terms.
...
def main():
    ...
    from config import TASKS as _TASKS
    parser.add_argument("--task", required=True, choices=list(_TASKS.keys()))
    ...
    parser.add_argument("--input-suffix", default="",
                        help="Suffix on the predictions file (e.g. '_audit' for RDS_predictions_audit.jsonl). "
                             "Output eval file gets the same suffix.")
    args = parser.parse_args()

    suffix = args.input_suffix
    pred_file = Path(RESULTS_DIR) / args.condition / f"{args.task}_predictions{suffix}.jsonl"
    eval_file = Path(RESULTS_DIR) / args.condition / f"{args.task}_eval{suffix}.jsonl"
